...
public class WorkingScreen extends ManagedScreen implements InputProcessor {
    ...
    public WorkingScreen(UselessProject game) {
        this.setGame(game);
    ...
    public void create() {
        ...
        imgBack = new Texture(Gdx.files.internal("img/menu/back-icon.png"));
        skin.add("imgBack", imgBack);
        ImageButton.ImageButtonStyle ibtnStyleHelp = new ImageButton.ImageButtonStyle();
        ibtnStyleHelp.up = skin.getDrawable("imgBack");
        btnBack = new ImageButton(ibtnStyleHelp);

        Label.LabelStyle lblStylecomment = new Label.LabelStyle();
        lblStylecomment.fontColor = Color.GREEN;
        lblStylecomment.font = skin.getFont("comment2");

        explainProblem=new Label("In this version resizing screen  \n  make fitviewport consistent",skin);
        ...
        if (getGame().isVertical()) {
            ...
            btnBack.setPosition(w - 296, hmiddle - 200);
